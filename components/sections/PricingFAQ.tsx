    ...
    <section id="pricing" className="relative overflow-hidden py-16 md:py-32 px-4 md:px-8"
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <div className="space-y-5 p-6 sm:p-10 md:p-14 rounded-[2rem]"
            style={{ background: 'rgba(255,255,255,0.05)', border: '2px solid rgba(212,168,83,0.25)', backdropFilter: 'blur(4px)' }}>

            {/* Ligne 1 */}
            <motion.div
              initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.05 }}
            ...
            {/* Ligne 2 */}
            <motion.div
              initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 }}
              className="flex justify-between items-center pb-5 px-2 cursor-default"
              style={{ borderBottom: '2px solid rgba(212,168,83,0.15)' }}
            >
              <span className="flex items-center gap-3" style={{ fontFamily: 'var(--font-baloo)', fontWeight: 700, fontSize: 'clamp(13px,1.3vw,20px)', color: '#f5ecd4' }}>
            ...
            {/* Total */}
            <motion.div
              initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: 0.22 }}
          ...
          {/* ── RIGHT : Stèle de prix — wrapper relatif pour que le chibi puisse déborder ── */}
      ...
      <section className="py-12 md:py-20 px-4 md:px-8 relative overflow-hidden"
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
            className="text-center mb-12" style={{ fontFamily: 'var(--font-baloo)', fontWeight: 900, fontSize: 'clamp(24px,4vw,52px)', color: '#1a2d4a', textTransform: 'uppercase', lineHeight: 1.1 }}>
            Tu hésites encore&nbsp;?
          </motion.h2>
          <div className="flex flex-col gap-3 relative">
            {/* Chibi pointeur hautain à gauche — en face du titre */}
            <motion.div
              className="absolute hidden xl:block z-0 pointer-events-none"
              style={{ right: 'calc(100% + 1rem)', top: 0, transform: 'translateY(-100%) scaleX(-1)', width: 'clamp(220px, 18vw, 320px)' }}
              animate={{ y: [0, -10, 0] }}
              transition={{ repeat: Infinity, duration: 3.8, ease: 'easeInOut', delay: 1 }}
            >
              <Image src="/pointeur-hautain.png" alt="" width={500} height={500} className="w-full h-auto drop-shadow-xl" />
            </motion.div>
            {faqItems.map((faq, i) => (
