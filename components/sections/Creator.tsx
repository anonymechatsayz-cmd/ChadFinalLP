...
function StatCard({ value, label, delay }: { value: string; label: string; delay: number }) {
  ...
  return (
    ...
    >
      <motion.div
        ref={tap.ref}
        animate={tap.hovered ? { y: -4 } : { y: 0 }}
        transition={{ type: 'spring', stiffness: 260, damping: 26 }}
        className="flex-1 text-center"
        style={{ background: '#FDFBF7', border: '3px solid #d4a017', borderRadius: 16, padding: 'clamp(16px, 2vw, 24px)', boxShadow: '4px 4px 0 rgba(212,160,23,0.35)', cursor: 'default' }}
        onPointerEnter={tap.onPointerEnter}
        onPointerLeave={tap.onPointerLeave}
      >
        <div style={{ fontFamily: 'var(--font-baloo)', fontWeight: 900, fontSize: 'clamp(28px, 3.5vw, 42px)', color: '#EC6426', lineHeight: 1 }}>{value}</div>
        <div style={{ fontFamily: 'var(--font-baloo)', fontWeight: 600, fontSize: 'clamp(12px, 1vw, 14px)', color: 'rgba(26,45,74,0.6)', textTransform: 'uppercase', letterSpacing: '.1em', marginTop: 4 }}>{label}</div>
      </motion.div>
    </motion.div>
  );
}
  ...
  return (
    <section className="py-24 px-4 md:px-8 text-white overflow-visible relative" style={{ background: '#1a2d4a', borderTop: '4px solid #d4a017' }}>
      ...
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-16 relative z-10">
        ...
        {/* ── GAUCHE : bloc illustré avec avatar — iris/focus depuis le centre ── */}
        <motion.div
        ...
        >
          ...
          <motion.h2
            className="flex flex-wrap justify-center md:justify-start items-center gap-x-3 gap-y-4 mb-8 uppercase"
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <StatCard value="100k+" label="Abonnés" delay={0.45} />
            <StatCard value="20M+"  label="Vues"     delay={0.55} />
          </div>
        </motion.div>
