          ...
          >
            <div style={{
              ...
              <ul className="flex flex-col gap-3 mt-4">
                {CONS_LEFT.map((txt, i) => (
                  <li key={i} className="flex items-start gap-3 px-2 py-1.5 rounded-md"
                    style={{ background: 'rgba(192,57,43,0.04)' }}>
                    <X className="w-5 h-5 shrink-0 mt-0.5" style={{ color: '#c0392b' }} />
                    <span style={{ fontFamily: 'var(--font-space)', fontSize: 'clamp(13px, 1vw, 15px)', color: 'rgba(42,30,18,0.80)', fontWeight: 500, lineHeight: 1.5 }}>{txt}</span>
                  </li>
                ))}
              </ul>
              {RULE}
