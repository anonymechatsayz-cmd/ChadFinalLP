...
function NavyCountdown() {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <p style={{ fontFamily:'var(--font-cinzel)', fontWeight:600, fontSize:10, letterSpacing:'.2em', color:'rgba(245,236,212,0.5)', textTransform:'uppercase' }}>
        L&apos;offre expire dans
      </p>
      <div className="flex items-center gap-2">
        {segs.map((seg, i) => (
          <div key={i} className="flex items-center gap-2">
            {i > 0 && <span style={{ color:'#EC6426', fontSize:20, fontFamily:'var(--font-cinzel)', opacity:0.6 }}>·</span>}
            <div className="flex flex-col items-center gap-0.5">
              <div style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(245,236,212,0.2)', borderRadius:8, padding:'7px 13px', minWidth:52 }}>
                <span style={{ fontFamily:'var(--font-cinzel)', fontWeight:900, fontSize:28, color:'#ffffff', display:'block', textAlign:'center', letterSpacing:'.04em', textShadow:'0 0 20px rgba(255,255,255,0.2)' }}>{seg.val}</span>
              </div>
              <span style={{ fontFamily:'var(--font-space)', fontSize:9, color:'rgba(245,236,212,0.4)', letterSpacing:'.1em', textTransform:'uppercase' }}>{seg.unit}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    <section className="relative overflow-hidden" style={{ background:'linear-gradient(180deg, #071229 0%, #0a1628 50%, #071229 100%)' }}>
      ...
      {NAVY_SYMS.map((m, i) => (
        <span key={i} className="absolute select-none pointer-events-none"
          style={{ top:`${m.top}%`, left:`${m.left}%`, fontSize:m.sz, opacity:m.op, color:'#f5ecd4', transform:`rotate(${m.rot}deg)`, fontFamily:'var(--font-cinzel)', fontWeight:700 }}>
          {m.s}
        </span>
      ))}
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-20 md:py-28 flex flex-col items-center text-center gap-8">
        ...
        <motion.div initial={{ opacity:0, y:-10 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.5 }}
          ...
          <span style={{ fontFamily:'var(--font-cinzel)', fontWeight:700, fontSize:10, color:'#EC6426', letterSpacing:'.2em', textTransform:'uppercase' }}>
            ✦ Offre de lancement ✦
          </span>
        </motion.div>
        ...
        <motion.h2 initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.6, delay:0.1 }}
          ...
          style={{ fontFamily:'var(--font-cinzel)', fontWeight:900, fontSize:'clamp(20px, 4vw, 58px)', color:'#f5ecd4', letterSpacing:'.06em', lineHeight:1.1, textTransform:'uppercase', textShadow:'0 4px 30px rgba(0,0,0,0.3)' }}>
        ...
        <motion.p initial={{ opacity:0 }} whileInView={{ opacity:1 }} viewport={{ once:true }} transition={{ duration:0.5, delay:0.2 }}
          style={{ fontFamily:'var(--font-space)', fontWeight:500, fontSize:'clamp(14px, 1.4vw, 18px)', color:'rgba(245,236,212,0.6)', maxWidth:520, lineHeight:1.6 }}>
        ...
        <motion.div initial={{ opacity:0, y:16 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.5, delay:0.3 }}
          ...
          <span style={{ fontFamily:'var(--font-cinzel)', fontWeight:900, fontSize:'clamp(42px, 5.5vw, 68px)', color:'#ffffff', textShadow:'0 0 40px rgba(255,255,255,0.2)', lineHeight:1 }}>
            {offerConfig.launchPrice}€
          </span>
          <div className="flex flex-col items-start gap-0.5">
            <span style={{ fontFamily:'var(--font-inter)', fontSize:16, color:'rgba(245,236,212,0.35)', textDecoration:'line-through' }}>{offerConfig.regularPrice}€</span>
            <span style={{ fontFamily:'var(--font-montserrat)', fontWeight:800, fontSize:11, color:'#EC6426', letterSpacing:'.08em' }}>-{offerConfig.discountPercent}%</span>
          </div>
        </motion.div>
