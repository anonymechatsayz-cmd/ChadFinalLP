    ...
    >
      ...
      {(['Α','Ω'].map((s, i) => (
        <span key={i} className="absolute select-none pointer-events-none"
          style={{ top: i === 0 ? '10%' : '65%', left: i === 0 ? '3%' : '92%', fontSize: 36, opacity: 1, color: 'rgba(26,45,74,0.12)', fontFamily: 'var(--font-baloo)', transform: `rotate(${i === 0 ? -15 : 18}deg)` }}>
          {s}
        </span>
      )))}
      ...
      >
        ...
        <div style={{
          ...
          {RULE}
          <p style={{ fontFamily: 'var(--font-cinzel)', fontWeight: 900, fontSize: 'clamp(18px, 2.5vw, 28px)', color: '#FFE234', letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 4, textShadow: '0 2px 4px rgba(0,0,0,0.9), 0 4px 12px rgba(0,0,0,0.8), 0 8px 32px rgba(0,0,0,0.6), 0 0 20px rgba(255,226,52,0.6)' }}>
            MATHS ULTIME
          </p>
          <h2 style={{
